...
union ult2_message {
	struct {
		__le64 opcode       :  6;    /* NNP_IPC_ULT2_OP */
...
#define H2C_OPCODE(name, val, type)    H2C_OPCODE_NAME(name) = (val), /* NNP_IGNORE_STYLE_CHECK */
...
#define C2H_OPCODE(name, val, type)    C2H_OPCODE_NAME(name) = (val),  /* NNP_IGNORE_STYLE_CHECK */
...
#define H2C_OPCODE(name, val, type)    NNP_STATIC_ASSERT((val) < 64, "opcode " #name " range overflow"); /* NNP_IGNORE_STYLE_CHECK */
#define C2H_OPCODE(name, val, type)    NNP_STATIC_ASSERT((val) >= 32 && (val) < 64, "opcode " #name " range overflow"); /* NNP_IGNORE_STYLE_CHECK */
#include "ipc_chan_h2c_opcodes.h"
#include "ipc_chan_c2h_opcodes.h"
#undef H2C_OPCODE
#undef C2H_OPCODE
...
#define C2H_OPCODE(name, val, type)  NNP_STATIC_ASSERT(sizeof(type) <= sizeof(__le64)*3, "Size of " #type " Must be LE 3*64!!"); /* NNP_IGNORE_STYLE_CHECK */
#include "ipc_chan_c2h_opcodes.h"
#undef C2H_OPCODE
