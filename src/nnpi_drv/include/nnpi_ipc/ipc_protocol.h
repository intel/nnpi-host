...
struct nnp_c2h_cpu_info {
	__le32 cpu_family;      /* for NNP = LceLake AIPG = 0x000906D0 */
	__u8  cpu_stepping;    /* CPU Stepping */
	__u8  cpu_sku;         /* CPU SKU */
	__le16 cpu_did;         /* for NNP range 0x4580-0x45FF */
...
struct nnp_c2h_system_info {
	...
	__le16 board_id; /* Board identification- for NNP RVP = 0x25 */
	__u8  fab_id;   /* Board Revision identification */
	__u8  bom_id;   /* Board Bill Of Material identification */
	__u8  platform_type;   /* For NNP RVP= 0x2, NNP M.2 = 0x3 */
